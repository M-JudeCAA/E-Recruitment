...
- `docker-compose.yml` exists but isn't wired up for local dev — use the two-terminal `npm run dev` flow in [SETUP.md](SETUP.md) instead. Hosted deployments: `render.yaml` (Render), `deploy/oracle/` (one-VM Docker Compose + Caddy) and `deploy/tunnel/` (this machine via Tailscale Funnel, staff on :8443; builds with `VITE_API_URL=same-origin` + `VITE_STAFF_PORT=8443` and uses the `vite preview` proxy) - see their READMEs.
